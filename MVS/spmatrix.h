...
template<typename R> double MatrixInfNorm (const CSRMatrix<R>&);
template<typename R> double MatrixFrobeniusNorm (const CSRMatrix<R>&);
...
#define FFM
	...
	friend double MatrixInfNorm FFM(const CSRMatrix<R>&);
	friend double MatrixFrobeniusNorm FFM(const CSRMatrix<R>&);
...
struct RowMat:public vector<map<int, R> >
{
	...
	bool SaveToFile(const char* filename) {
		// save the non-zero elements of matrix to file
		fstream out(filename, ios::out);
		if (!out) {
			cout << "Can not open " << filename << " to save matrix." << endl;
			return false;
		}
		for (int i = 0; i < nRow(); ++ i) {
			map<int, R>& crow = (*this)[i];
...
template <typename R>
double MatrixInfNorm(const CSRMatrix<R>& matA)
{
	double InfNorm = -1;
	for (int i = 0; i < matA.nRow(); ++ i) {
		double row_sum = 0.0;
		for (int j = 0; j < matA.nCol(); ++ j) {
			row_sum += std::abs(matA.getElementV(i,j));
		}
		InfNorm = std::max(InfNorm, row_sum);
	}
	return InfNorm;
}

template <typename R>
double MatrixFrobeniusNorm(const CSRMatrix<R>& matA)
{
	double FrobeniusNorm = 0.0;
	CSRMatrix<R> matAT, matATA; CSRMatrixTranspose(matA, matAT);
	Mul2Matrices(matAT, matA, matATA);
	for (int i = 0; i < matATA.nRow(); ++ i) {
		FrobeniusNorm += std::pow(matATA.getElementV(i, i), 2.0);
	}
	return std::sqrt(FrobeniusNorm);
}
