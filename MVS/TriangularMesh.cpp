...
bool TriangularMesh::LoadVertexColorFile(const char* filename, double m_sigma)
{
	...
	MyMesh org_mesh = this->m_ObjTriMesh; MyMesh fit_mesh = this->m_ObjTriMesh;		
	org_mesh.request_vertex_colors();			fit_mesh.request_vertex_colors();	
	double original_intensity, fitted_intensity;
	for (int i = 0; i < c_size; i ++) {
		fin >> fitted_intensity >> original_intensity;
		org_mesh.set_color(MyMesh::VertexHandle(i), MyMesh::Color(original_intensity,original_intensity,original_intensity));
		fit_mesh.set_color(MyMesh::VertexHandle(i), MyMesh::Color(fitted_intensity,fitted_intensity,fitted_intensity));
		m_vertices[i].ori_intensity = original_intensity;
		m_vertices[i].fit_intensity = fitted_intensity;
		m_vertices[i].intensity = original_intensity;
		//// if the difference between ori and fit intensity is large, then treat as occlusion region
		//if (abs(fitted_intensity-original_intensity)>0.68*max(fitted_intensity, original_intensity)) {
		//	m_vertices[i].intensity = fitted_intensity;
		//}
	...
	OpenMesh::IO::write_mesh(fit_mesh, Obj_prefix+"MeshColorFit.off", write_options);
	OpenMesh::IO::write_mesh(this->m_ObjTriMesh, Obj_prefix+"MeshColorHyb.off", write_options);
	return true;
}
...
void TriangularMesh::ALM_TVU_MeshRefinement(string meshname, double fidParam, double m_beta, double ndp_eta, double ndf_eta, double varsigma, double penParam, double regParam = 1.0, double lapParam = 100, bool UseTVU = true, bool UseTVNorm = false, int iter_step = 0, bool UseFaceArea = false, bool UseMatlabSolver = true)
{
	...
	bool UseLambert = (m_beta>0)?true:false;	bool UseLaplace = (lapParam>0)?true:false; bool UseFidelity = fidParam>0?true:false;
	bool UsePointLD = ndp_eta>0?true:false;		bool UseFaceND = ndf_eta>0?true:false;
	cout << endl << "Start the mesh refinement process: ";	UseFidelity?cout<<"Fid "<<fidParam<<" ":cout<<" ";
	UseLambert?cout<<"Lam "<<m_beta<<" ":cout<<" ";			UseLaplace?cout<<"Lap "<<lapParam<<" ":cout<<" ";
	UsePointLD?cout<<"LDP "<<ndp_eta<<" ":cout<<" ";		UseFaceND?cout<<"NDF "<<ndf_eta<<" ":cout<<" "; 
	...
	do { // start the iteration
		...
		for (int iter = 0; iter < innerL; ++ iter) {
			...
			CalculateEnergyTerm(m_f, t_energy, UseFidelity, UseLaplace, UseLambert, UsePointLD, UseFaceND, UseTVNorm, UseTVU);
			total_energy = 0.0;
			for (int k = 0; k < t_energy.size(); ++ k) {
				total_energy += t_energy[k];
			}
			t_energy.push_back(total_energy);			energy_result.push_back(t_energy); 
			cout << "00: Sum: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[t_energy.size()-1]; 
			int eidx = 0;
			if (UseFidelity) {
				cout << " U: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++]; 
			}
			if (UseLaplace) {
				cout << " LAP: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
			}
			if (UseLambert) {
				cout << " Lam: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
				//cout << " = " << 0.5*m_beta*CalculateNEnergy(T_Mesh, vec_n_energy,UseFaceArea);
			}
			if (UsePointLD) {
				cout << " LDP: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
			}
			if (UseFaceND) {
				cout << " NDF: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
			}
			if (UseTVNorm) {
			...
			sprintf(buffer, "Results\\LMResults\\%sLMResult%d%s%s%s%s%s%s%s%s_%.0f_%.0f_%.5f_%.0f_%.0f_%.0f_%.0f_%.0f_%d_%d.off", meshname.c_str(), iter_step, 
				UseFidelity?"Fid":"_", UseLambert?"Beta":"_", UsePointLD?"NDP":"_", UseFaceND?"NDF":"_", UseLaplace?"Lap":"_", UseTVU?"TVU":"_",UseTVNorm?"TVNorm":"_", UseFaceArea?"FA":"_", fidParam, m_beta, penParam, regParam, ndp_eta, ndf_eta, lapParam, varsigma, outL, 0);
			write_options.set(OpenMesh::IO::Options::VertexColor); 
			...
			while (iteration++ < maxStep) {
				...
				if (UseMatlabSolver) {
					gmm::copy(gmm::transposed(mat_JTJ), JtJ);
					...
					for (gmm::linalg_traits<RowSparseMatrix>::row_iterator rIt = gmm::linalg_traits<RowSparseMatrix>::row_begin(mat_JTJ);
						rIt != gmm::linalg_traits<RowSparseMatrix>::row_end(mat_JTJ); ++ rIt, ++ r) {
							...
							for (; it != ite; ++ it) {
								RM_A(r, it.index()) = mat_JTJ(r, it.index());
							}
							b[r] = m_b[r];
					}
					solver.getMatA() = RM_A;
				...
				if (normH < epsilon2*(sqrt(normX) + epsilon2)) {
					...
					t_energy.push_back(total_energy);			energy_result.push_back(t_energy); 
					cout << " Sum: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[t_energy.size()-1];// << " = " << eNew; 
					int eidx = 0;
					if (UseFidelity) {
						cout << " U: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
					...
					if (UsePointLD) {
						cout << " NDP: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
					...
					if (UseTVNorm) {
						...
						//	nuz[v_it.handle().idx()] = T_Mesh.normal(v_it.handle()).data()[2];
						//}
						//cout << " = " << 0.5*penParam*CalculateVTVNormEnergy(T_Mesh, nux, nuy, nuz,UseFaceArea);
					} 
					if (UseTVU) {
						cout << " TVU: " <<setprecision(6)<<setw(6)<<setiosflags(ios::left)<< t_energy[eidx++];
						//for (MyMesh::VertexIter v_it = T_Mesh.vertices_begin(); v_it != T_Mesh.vertices_end(); ++ v_it) { 
					...
					if (iteration%1 == 0) {
						sprintf(buffer, "Results\\LMResults\\%sLMResult%d%s%s%s%s%s%s%s%s_%.0f_%.0f_%.5f_%.0f_%.0f_%.0f_%.0f_%.0f_%d_%d.off", meshname.c_str(), iter_step, 
							UseFidelity?"Fid":"_", UseLambert?"Beta":"_", UsePointLD?"NDP":"_", UseFaceND?"NDF":"_", UseLaplace?"Lap":"_", UseTVU?"TVU":"_",UseTVNorm?"TVNorm":"_", UseFaceArea?"FA":"_", fidParam, m_beta, penParam, regParam, ndp_eta, ndf_eta, lapParam, varsigma, outL, iteration);
						write_options.set(OpenMesh::IO::Options::VertexColor); 
						if ( !OpenMesh::IO::write_mesh(T_Mesh, string(buffer), write_options) ) {
							std::cerr << "Cannot write mesh to file " << buffer << std::endl;
						}
			...
			sprintf(buffer, "Results\\LMResults\\%sLMResult_Final%d%s%s%s%s%s%s%s%s_%.0f_%.0f_%.5f_%.0f_%.0f_%.0f_%.0f_%.0f_%d.off", meshname.c_str(), iter_step, 
				UseFidelity?"Fid":"_", UseLambert?"Beta":"_", UsePointLD?"NDP":"_", UseFaceND?"NDF":"_", UseLaplace?"Lap":"_", UseTVU?"TVU":"_",UseTVNorm?"TVNorm":"_", UseFaceArea?"FA":"_",
...
void TriangularMesh::CalculateEnergyTerm(DenseMatrix mat_f, vector<double>& energy, bool UseFidelity, bool UseLaplace, bool UseLambert, bool UsePointLD, bool UseFaceND, bool UseTVNorm, bool UseTVU)
{
...
void TriangularMesh::TV_JacobianMatrix_Construction(MyMesh& T_Mesh, RowSparseMatrix& mat_J, DenseMatrix& mat_f, double fidParam, double m_beta, double ndp_eta, double ndf_eta, 
	double varsigma, double penParam, double lapParam, vector<VECTOR3D> &px, vector<VECTOR3D> &py, vector<VECTOR3D> &pz, 
	vector<VECTOR3D> &lambda_x, vector<VECTOR3D> &lambda_y, vector<VECTOR3D> &lambda_z, bool UseTVU = false, bool UseTVNorm = false, bool UseFaceArea = false)
{
	...
	int dimension = UseFidelity*m_vnum*3 + UseLaplace*m_vnum*3 + UseLambert*m_vnum + UsePointLD*T_Mesh.n_edges()*3 + UseFaceND*T_Mesh.n_edges()*3 + UseTVNorm*m_trinum*9 + UseTVU*m_trinum*9;
	...
	if (UseFidelity) {
		for (MyMesh::VertexIter v_it = T_Mesh.vertices_begin(); v_it != T_Mesh.vertices_end(); ++ v_it) {
			...
			mat_J(vertex_id*3+0+Start_id, vertex_id+m_vnum*0) += area_scale*fid_scale*1.0;
			mat_J(vertex_id*3+1+Start_id, vertex_id+m_vnum*1) += area_scale*fid_scale*1.0;
			mat_J(vertex_id*3+2+Start_id, vertex_id+m_vnum*2) += area_scale*fid_scale*1.0;
			OpenMesh::Vec3f temp = T_Mesh.point(v_it.handle());

			mat_f(vertex_id*3+0+Start_id, 0) += area_scale*fid_scale*(T_Mesh.point(v_it.handle())[0] - m_vertices[vertex_id].ref_x);
			mat_f(vertex_id*3+1+Start_id, 0) += area_scale*fid_scale*(T_Mesh.point(v_it.handle())[1] - m_vertices[vertex_id].ref_y);
			mat_f(vertex_id*3+2+Start_id, 0) += area_scale*fid_scale*(T_Mesh.point(v_it.handle())[2] - m_vertices[vertex_id].ref_z);
		} 
		Start_id += m_vnum*3;
	}
	...
	if (UsePointLD) {
		...
		for (MyMesh::EdgeIter e_it = T_Mesh.edges_begin(); e_it != T_Mesh.edges_end(); ++ e_it) {
			...
			mat_f(e_it.handle().idx()*3+2+Start_id, 0) += area_scale*ndp_scale*(m_vertices[iid].light_z - m_vertices[jid].light_z);
		}
...
//		}
		...
		for (MyMesh::EdgeIter e_it = T_Mesh.edges_begin(); e_it != T_Mesh.edges_end(); ++ e_it) {
			...
			for (int k = 0; k < 3; k++) {
				for (it = vect_const_begin(gmm::mat_const_row(gradJ, iid*3+k)); it != vect_const_end(gmm::mat_const_row(gradJ, iid*3+k)); ++ it) {
					mat_J(e_it.handle().idx()+Start_id, it.index()) += area_scale*ndp_scale*LightInfo[k]*gradJ(iid*3+k, it.index());
				}
				for (it = vect_const_begin(gmm::mat_const_row(gradJ, jid*3+k)); it != vect_const_end(gmm::mat_const_row(gradJ, jid*3+k)); ++ it) {
					mat_J(e_it.handle().idx()+Start_id, it.index()) -= area_scale*ndp_scale*LightInfo[k]*gradJ(jid*3+k, it.index());
				}
				mat_J(e_it.handle().idx()+Start_id, 3*m_vnum+k) += area_scale*ndp_scale*(T_Mesh.normal(MyMesh::VertexHandle(iid))[k] - T_Mesh.normal(MyMesh::VertexHandle(jid))[k]); 
			}
			mat_f(e_it.handle().idx()+Start_id, 0) += area_scale*ndp_scale*(OpenMesh::dot(LightInfo, (T_Mesh.normal(MyMesh::VertexHandle(iid)) - T_Mesh.normal(MyMesh::VertexHandle(jid)))) - (m_vertices[iid].intensity - m_vertices[jid].intensity));
		}
	...
	if (UseFaceND) {
		...
		for (MyMesh::EdgeIter e_it = T_Mesh.edges_begin(); e_it != T_Mesh.edges_end(); ++ e_it) {
			...
			//for (int k = 0; k < 3; ++ k) {
			//	mat_J(e_it.handle().idx()+Start_id, ida+m_vnum*0) += area_scale*ndf_scale*LightInfo[k]*n2ax[k];
			//	mat_J(e_it.handle().idx()+Start_id, ida+m_vnum*1) += area_scale*ndf_scale*LightInfo[k]*n2ay[k];
			//	mat_J(e_it.handle().idx()+Start_id, ida+m_vnum*2) += area_scale*ndf_scale*LightInfo[k]*n2az[k];

			//	mat_J(e_it.handle().idx()+Start_id, idb+m_vnum*0) += area_scale*ndf_scale*LightInfo[k]*n2bx[k];
			//	mat_J(e_it.handle().idx()+Start_id, idb+m_vnum*1) += area_scale*ndf_scale*LightInfo[k]*n2by[k];
			//	mat_J(e_it.handle().idx()+Start_id, idb+m_vnum*2) += area_scale*ndf_scale*LightInfo[k]*n2bz[k];

			//	mat_J(e_it.handle().idx()+Start_id, idc+m_vnum*0) += area_scale*ndf_scale*LightInfo[k]*n2cx[k];
			//	mat_J(e_it.handle().idx()+Start_id, idc+m_vnum*1) += area_scale*ndf_scale*LightInfo[k]*n2cy[k];
			//	mat_J(e_it.handle().idx()+Start_id, idc+m_vnum*2) += area_scale*ndf_scale*LightInfo[k]*n2cz[k];

			//	mat_J(e_it.handle().idx()+Start_id, k+m_vnum*3) += area_scale*ndf_scale*Nijk[k];
			//}
			...
			//mat_f(e_it.handle().idx()+Start_id, 0) += area_scale*ndf_scale*(OpenMesh::dot(LightInfo, (T_Mesh.normal(fh1) - T_Mesh.normal(fh2))) - (Cf1 - Cf2));
...
void TriangularMesh::GradientTesting(double h = 0.0005, bool TestTV = false, int choice = 0)
{
	...
	if (TestTV) {
		...
		case 1:
			...
			break;
		case 2:
			ndp_eta = 1.0; m_beta = 1.0; lapParam = 1.0;
			break;
		case 3:
...
/*   Iteration        */
	do
	{
		...
		for(l=0;l<innerL;l++)
		{
...
#ifdef _DEBUG
			sprintf(buffer, "Results\\NEnergy_%d_%d_%.0f_%.0f_%.3f.txt", outL, iter, fidParam, m_beta, n_energy);
			fstream nof(buffer,std::ios::out);
			if (!nof) {
			...
			cout << "The current energy is V:" << 0.5*fidParam*v_energy << ", N:" << m_beta*n_energy << ", TV: " << penParam*tv_energy << "; Sum: " 
				<< 0.5*fidParam*v_energy + m_beta * n_energy + penParam * tv_energy << endl;
			OpenMesh::IO::Options write_options;
			sprintf(buffer, "Results\\ALMMeshResult_%d_%.0f_%.0f_%.3f.off", outL, fidParam, m_beta, penParam);
			if ( !OpenMesh::IO::write_mesh(T_Mesh, string(buffer), write_options) ) {
				std::cerr << "Cannot write mesh to file " << buffer << std::endl;
			}
		/*  Solve the u-sub problem  (solve for ur,ug,ub).   */
...
//void TriangularMesh::ALM_TVNorm_MeshRefinement(double fidParam, double m_beta, double penParam, double regParam = 1.0, bool IsComplexVersion = false)
//{
//	unsigned long nver = m_vnum, ntri = m_trinum; int itmax = 300, itol = 1;//1,2,3, or 4.
//	double tol = 1.0e-15;
//	vector<double>ux, uy, uz; ux.resize(nver); uy.resize(nver); uz.resize(nver);
...
//	/*   Parameters setting  */
//	unsigned int innerL = 1; int outL = -1;
//	double outTole = 1.0e-10, stoppingCond;// fidParam = 1000, m_beta = 0, penParam = 0.01, regParam = 1,
...
//			vector<double> t_energy; t_energy.clear(); t_energy.push_back(0.5*fidParam*v_energy); t_energy.push_back(m_beta*n_energy);
//			t_energy.push_back(regParam*tv_energy); t_energy.push_back(0.5*fidParam * v_energy + m_beta * n_energy + regParam * tv_energy);
//			energy_result.push_back(t_energy);
//			cout << "The current energy is V:" << 0.5*fidParam*v_energy << ", N:" << m_beta*n_energy << ", TV: " << regParam*tv_energy << "; Sum: " 
//				<< 0.5*fidParam * v_energy + m_beta * n_energy + regParam * tv_energy << endl;
//			OpenMesh::IO::Options write_options;
//			write_options.set(OpenMesh::IO::Options::VertexNormal); 
//			sprintf(buffer, "Results\\TVNorm_MeshResult_%d_%.0f_%.0f_%.3f_%s.off", outL, fidParam, m_beta, penParam, IsComplexVersion?"Complex":"Simple");
//			if ( !OpenMesh::IO::write_mesh(T_Mesh, string(buffer), write_options) ) {
//				std::cerr << "Cannot write mesh to file " << buffer << std::endl;
//			}
//
...
//			TVNorm_JacobianMatrix_Construction(T_Mesh, mat_J, m_f, fidParam, m_beta, penParam, px, py, pz, lambda_x, lambda_y, lambda_z, IsComplexVersion);
//
//			eOld = gmm::vect_norm2_sqr(m_f);
//			RowSparseMatrix mat_JTJ(dimension, dimension);
//			RowSparseMatrix mat_A(dimension, dimension);
...
//
//
//void TriangularMesh::TVNorm_JacobianMatrix_Construction(MyMesh& T_Mesh, RowSparseMatrix& mat_J, std::vector<double>& mat_f, double fidParam, double m_beta, double penParam, vector<VECTOR3D> &px, vector<VECTOR3D> &py, vector<VECTOR3D> &pz, vector<VECTOR3D> &lambda_x, vector<VECTOR3D> &lambda_y, vector<VECTOR3D> &lambda_z, bool IsComplexVersion = false)
//{
//	// construct the jacobian matrix according to the new model
...
//	double fid_scale = sqrt(fidParam*0.5), beta_scale = sqrt(m_beta*0.5), pen_scale = sqrt(penParam*0.5);
//
//	OpenMesh::Vec3f pointA , pointB , pointC; int ida, idb, idc;
//	for (MyMesh::VertexIter v_it = T_Mesh.vertices_begin(); v_it != T_Mesh.vertices_end(); ++ v_it) {
//		const int vertex_id = v_it.handle().idx(); double diag_value = 0.0;
...
void TriangularMesh::MeshRefinement(double alpha, double beta, double eta, double varsigma, bool IsComplexVersion = false)
{
	...
	/*   Parameters setting  */
	double outTole = 1.0e-10;// fidParam = 1000, m_beta = 0, penParam = 0.01, regParam = 1,
